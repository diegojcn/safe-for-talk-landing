...
const API_BASE =
  process.env.VITE_SAFE_4_TALK_API_URL ??
...
/**
 * The same mapping the page uses, duplicated here on purpose: this file runs on Vercel's
 * runtime and cannot import from the Vite bundle. Kept small for that reason — the preview
 * needs the language name, not the whole page.
 */
const LANGUAGE_NAMES = {
  ...
  de: 'alemão', german: 'alemão',
  pt: 'português', portuguese: 'português',
}

const languageLabel = (code) =>
  LANGUAGE_NAMES[String(code ?? '').trim().toLowerCase()] ?? String(code ?? '').trim()
...
export function injectTeacherMeta(html, teacher, canonicalUrl) {
  // Not "aulas de inglês": teachers pick the languages they teach, and a Spanish teacher
  // shared on Instagram would have been announced as an English one.
  const languages = (teacher.languages ?? [])
    .map((language) => languageLabel(language.languageCode))
    .filter(Boolean)
  const subject = languages.length > 0 ? languages.join(' e ') : 'idiomas'
  const title = `${teacher.displayName} — aulas de ${subject} 1:1 | Safe 4 Talk`
  const description = teacher.bio
    ? teacher.bio.slice(0, 150)
    : `Agende uma aula de ${subject} 1:1 com ${teacher.displayName} no Safe 4 Talk.`
